...
- [x] Matrix A shown beside the buttons (`A = ` LaTeX + "columns = where the x-direction and y-direction vectors land" + x-direction/y-direction columns line). **This session:** all i-hat/j-hat basis-vector language replaced with plain "x-direction vector / y-direction vector" throughout Topic 2 (the top INTRO constant in `__init__.py` AND `screen_2d.py`'s caption + columns line); the INTRO now explains WHY these two vectors matter (every point is built from them, so where they land tells you where everything lands) and includes a concrete Scale x2 example (x-direction -> (2,0), y-direction -> (0,2), everything doubles). No hats anywhere; "vector" used consistently (never "arrow"). i-hat/j-hat notation dropped entirely per the columns-mean-different-things-in-different-contexts concern (system columns are x1/x2 coefficients; deferring the unifying notation)
