...
def render_row():
    ...
    st.markdown("**Reduce it yourself: find the row space.**")
    st.markdown(
        "Row operations never change the row space -- mixing rows just rewrites the "
        "same rules -- so row-reduce A to reduced form. The nonzero rows that remain "
        "are the row space (its cleanest basis). Reduce A below, then read the answer "
        "underneath."
    )
    _ROW_A = [[1, 2, 1, 1], [1, 3, 2, 4], [2, 5, 3, 5], [0, 1, 1, 3]]
    if st.session_state.get("t06_row_wb_last") is None:
        space_load("t06_row_wb", _ROW_A)
        st.session_state["t06_row_wb_last"] = "loaded"
    space_workbench("t06_row_wb", 4, matrix_label="A")

    st.markdown("**Now read the row space off your reduced form.**")
    # reduced form: nonzero rows purple, zero rows gray
    st.latex(
        r"\begin{bmatrix}"
        r"\color{#9775fa}{1} & \color{#9775fa}{0} & \color{#9775fa}{-1} & \color{#9775fa}{-5} \\"
        r"\color{#9775fa}{0} & \color{#9775fa}{1} & \color{#9775fa}{1} & \color{#9775fa}{3} \\"
        r"\color{gray}{0} & \color{gray}{0} & \color{gray}{0} & \color{gray}{0} \\"
        r"\color{gray}{0} & \color{gray}{0} & \color{gray}{0} & \color{gray}{0}"
        r"\end{bmatrix}"
    )
    st.caption(
        "Purple = the two nonzero rows (the genuinely different rules). Gray = the two "
        "zero rows, which were redundant and drop out. UNLIKE the column space (where "
        "you take columns from the ORIGINAL matrix), the row space is read straight "
        "from the REDUCED rows."
    )
    st.latex(
        r"\text{any rule} = \underbrace{\color{#9775fa}{"
        r"r_1\begin{bmatrix} 1 & 0 & -1 & -5 \end{bmatrix}"
        r" + r_2\begin{bmatrix} 0 & 1 & 1 & 3 \end{bmatrix}}}_{\text{row space}}"
    )
    st.caption(
        "Two nonzero rows, so the row space is 2-dimensional -- its dimension is the "
        "rank, 2, the same rank as the column space. Those two rows are a basis for it."
    )
    st.markdown("---")
    ...
    st.markdown("**Smoothie general solution (all three free-variable directions):**")
    st.latex(_SMOOTHIE_PARAMETRIC_LATEX)
